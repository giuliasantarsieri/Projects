...
# Importation du module MPI et mise en place de l'environnement parallèle
from mpi4py import MPI

# définition du communicateur de couple pour relier calcul et affichage:
...
status_couple = 0
comm = comm_couple.Split(status_couple, 0)
size_couple = comm_couple.size

fileName = f"sortie_plot.txt"
file = open(fileName, mode="w")
file.write(f"\tRang couple : {rank_couple}\n")
file.write(f"\tTaille du processus couple: {size_couple}\n")

p = int(sqrt(size_couple-1))
...
if __name__ == '__main__':
    ...
    # récéption des dimensions des grilles locales:
    ny_loc = comm_couple.recv(source=1,tag = 11)
    nx_loc = comm_couple.recv(source = 1, tag = 14)
    
    # Initialisation de la grille globale :
    ...
    while(True):
        #time.sleep(0.05)
        t1 = time.time()
        cells_loc = np.zeros((ny_loc-2,nx_loc-2),dtype=np.uint8)
        for s in range(1,size_couple) :
        
            # récéption de la taille de diff:
            taille = comm_couple.recv(source=s, tag=12)
            diff = np.zeros(taille,dtype = np.uint32)
            file.write(f"\ttaille diff: {taille}\n")
            
            # récéption de diff et des cellules locales
            comm_couple.Recv(diff,source=s,tag=13)
            comm_couple.Recv(cells_loc,source =s ,tag = 10)
            file.write(f"\tdiff: {diff}\n")
            
            # mise à jour des cellules pour la grille globale:
            grid.cells[(ny_loc-2)*((s-1)//p):(ny_loc-2)*((s-1)//p)+(ny_loc-2), (nx_loc-2)*((s-1)%p):(nx_loc-2)*((s-1)%p)+(nx_loc-2)] = cells_loc
            file.write(f"\ttaille cells: {grid.dimensions}\n")
        appli.draw(diff)
        t2 = time.time()
        t3 = time.time()
